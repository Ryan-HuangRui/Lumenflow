...
4. For tutorials:
   ...
   - Generate one private evidence card per successful tutorial.
   - Merge evidence into source-agnostic reusable style/method knowledge and refresh the public index.
...
Refresh private evidence cards and rebuild reusable knowledge after new recipes are created:
...
10. Run `scripts/generate_tutorial_style_cards.py` to create or refresh one private evidence card per tutorial recipe under `knowledge/style_cards/tutorial_derived/`.
11. Run `scripts/build_style_family_layer.py` to classify and merge evidence into source-agnostic reusable cards under `knowledge/style_families/`, refresh `knowledge/style_library_index.json`, and write ignored provenance to `knowledge/private_provenance/style_source_map.json`.
12. Treat video-level cards as private build evidence, never as runtime style knowledge.
13. Let `develop-photos` retrieve only reusable semantic cards and infer concrete settings from the target photo.
14. Review new or materially changed reusable cards before committing them.
...
- Keep generated tutorial transcripts, recipes, tutorial-derived evidence cards, private provenance, and concrete tutorial source whitelists out of public/plugin distributions.
...
- Keep source evidence separated from reusable knowledge:
  - `knowledge/style_families/*.json` contains tracked, source-agnostic visual/method knowledge for runtime retrieval.
  - `knowledge/style_cards/tutorial_derived/*.json` contains ignored private evidence, one card per successful source video.
  - `knowledge/private_provenance/style_source_map.json` is the ignored audit mapping.
  - `knowledge/style_library_index.json` routes agents only to reusable knowledge.
- Never publish platform/video IDs, source titles, source URLs, transcript paths, transcript excerpts, timestamps, or per-video style IDs in reusable cards or the index.
...
- Preserve provenance in the ignored private source map; public reusable cards must remain source-agnostic.
...
knowledge/private_provenance/style_source_map.json
