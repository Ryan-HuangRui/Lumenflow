...
The darktable feasibility gate is itself versioned as `lumenflow.darktable_probe.v1`. It runs a real RAW export with a temporary config/cache, an in-memory library, and sidecar writes disabled, then compares RAW and sidecar state and fingerprints the output. A command/version check alone is inconclusive. The verified adapter includes a state-bound preview provider, exact-XMP replay, and a fail-closed 5.4.1 dynamic codec for exposure, temperature, sigmoid, filmic RGB, color balance RGB, crop, highlights, demosaic, profiled denoise, lens correction, sharpening, diffuse/sharpen, tone equalizer, color equalizer, and perspective/rotation. For a bare RAW, the preview workflow materializes the codec-owned `xmp_version=5` empty history under the output root rather than beside the source. JPEG/PNG/TIFF/OpenEXR output controls are bounded and live-tested. The crop struct is engine-native only; generic pixel-crop composition, masks/blends, catalog writes, and unlisted modules remain unsupported.
